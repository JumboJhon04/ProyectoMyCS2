...
export default function UserPanelAdmin(){
  const [activeTab, setActiveTab] = useState('responsables'); // 'responsables' o 'todos'
  ...
  useEffect(() => {
    // Solo cargar responsables cuando estemos en esa pestaña
  ...
  return (
    <div className="upa-wrapper upa-root">
      {/* Tabs */}
      <div className="upa-tabs">
        <button
          className={`upa-tab ${activeTab === 'responsables' ? 'active' : ''}`}
          onClick={() => setActiveTab('responsables')}
        >
          Responsables
        </button>
        <button
          className={`upa-tab ${activeTab === 'todos' ? 'active' : ''}`}
